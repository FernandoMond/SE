...
int div (int x, int y)
{	if (x == y){
		x++; //No doy dividido correctamente cuando son el mismo numero, quick fix
	}	
	int resultado =0;
	asm(
		"CMP	%[X], %[Y]\n" //Miramos si se puede dividir una primera vez.
		"Bls	endfun\n" //Si no se puede salimos
	"loop:\n"
		"SUB	%[X], %[X], %[Y]\n" //Restamos el divisor
		"ADD	%[AUX], %[AUX],#1\n" //Sumamos ya que cupo
		"CMP	%[Y], %[X]\n" //Miramos si se puede seguir dividiendo
		"Bls	loop\n" //Si se puede seguimos
	"endfun:\n"
	: [AUX] "+l" (resultado), [X] "+l" (x) //Variables de salida
	: [Y] "l" (y) //Variables de entrada
	);
	return resultado;
}
...
void lcd_display_time(uint8_t value1, uint8_t value2)
{
  if ((value1 > 99) | (value2 > 99)) {
   ...
   lcd_display_error(0x10);
    ...
    lcd_set(div(value1,10), 1);
    lcd_set(value1 % 10, 2);
    lcd_set(div(value2,10), 3);
    lcd_set(value2 % 10, 4);
    SegLCD_Col_On();
  }
}
